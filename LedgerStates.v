...
| sf_rights_consumed_cons beta h inpr r1 obl r2:
    sf_rights_consumed b alpha f inpr r1 ->
    In (h,(obl,rights b r2 alpha)) (f beta) ->
    sf_rights_consumed b alpha f ((beta,h)::inpr) (r1 + r2)
| sf_rights_consumed_skip beta h inpr r1 obl u :
    sf_rights_consumed b alpha f inpr r1 ->
    In (h,(obl,u)) (f beta) ->
...
(forall alpha b h obl u beta h' obl' u' beta', In (h,(obl,owns b beta u)) (f alpha) -> In (h',(obl',owns b beta' u')) (f alpha) -> h = h' /\ obl = obl' /\ u = u' /\ beta = beta')
.
...
Definition statefun_rights_balanced (f:statefun) (alpha:addr) (b:bool) (inpl:list addr_assetid) (outpl:list addr_preasset) : Prop :=
   (forall (rtot1 rtot2 : nat) (h : hashval) (obl : obligation) 
    ...
    In (h, (obl, owns b beta (Some u))) (f alpha) ->
...
((forall alpha b,
    ...
    ((~exists h' obl' beta', In (h',(obl',owns b beta' None)) (f alpha))
     /\
 ...
 (forall alpha b beta h obl n,
    ...
    In (h,(obl,rights b n alpha)) (f beta) ->
    rights_mentioned alpha b (tx_outputs tx)))
/\
...
(forall obl nonce d alpha,
   ...
   exists beta h obl,
     ...
     In (beta,h) (tx_inputs tx)
     /\
     In (h,(obl,intention alpha)) (f beta))
/\
...
(forall alpha b obl beta r,
    ...
    ((exists h beta' obl' r', In (h,(obl',owns b beta' r')) (f alpha)
...
(forall alpha b,
   In alpha (output_uses b (tx_outputs tx)) ->
   ~(exists h' beta' obl' r', In (h',(obl',owns true beta' r')) (f alpha)) ->
...
(forall alpha h obl u,
   In (alpha,h) (tx_inputs tx) ->
   In (h,(obl,bounty u)) (f alpha) ->
   exists h' obl' beta r,
     In (alpha,h') (tx_inputs tx)
     /\
     In (h',(obl',owns true beta r)) (f alpha)
     /\ 
...
Lemma statefun_supports_tx_owns_trans (f:statefun) (tx:Tx) fee rew :
  sf_valid f ->
  statefun_supports_tx f tx fee rew ->
  forall alpha b obl beta u h obl' gamma v, In (alpha,(obl,owns b beta u)) (tx_outputs tx) -> In (h,(obl',owns b gamma v)) (f alpha) -> In (alpha,h) (tx_inputs tx).
intros Hf [_ [_ [_ [Hs5 _]]]] alpha b obl beta u h obl' gamma v H1 H2.
destruct (Hs5 alpha b obl beta u H1) as [[k [beta' [obl2 [w [H3 H4]]]]]|[H3 H4]].
- destruct Hf as [_ [_ [_ [_ Hf5]]]].
  destruct (Hf5 alpha _ _ _ _ _ _ _ _ _ H2 H3) as [H5 _].
  subst k. exact H4.
- exfalso. apply H3. exists h. exists gamma. exists obl'. exists v.
  exact H2.
Qed.
...
Theorem sf_tx_valid_thm m (f:statefun) (tx:Tx) fee rew :
  sf_valid f ->
  tx_valid m tx ->
  statefun_supports_tx f tx fee rew ->
        ...
        - intros h H1 [alpha H2].
          ...
          + apply remove_assets_iff in H2. destruct H2 as [H3 H4].
            ...
            * exfalso. apply H4. apply get_spent_iff. simpl.
              ...
              congruence.
        - intros alpha b h obl beta u h' obl' beta' u' H1 H2.
          apply (tx_statefun_trans_lem_iff f inpl) in H1.
          apply (tx_statefun_trans_lem_iff f inpl) in H2.
          destruct H1 as [[H1a H1b]|[i [H1a H1b]]];
            destruct H2 as [[H2a H2b]|[i' [H2a H2b]]].
          * now apply (Hf5 alpha b).
          * exfalso. apply H1b. apply nth_error_In in H2a.
            exact (statefun_supports_tx_owns_trans _ _ _ _ Hf Hs alpha b _ _ _ _ _ _ _ H2a H1a).
          * exfalso. apply H2b. apply nth_error_In in H1a.
            exact (statefun_supports_tx_owns_trans _ _ _ _ Hf Hs alpha b _ _ _ _ _ _ _ H1a H2a).
...
- simpl. rewrite <- (IH (S i)). unfold statefun_totalunits.
  ...
  + rewrite (asset_value_sum_novalue_cons _ _ E1).
    destruct u as [n|m|alpha v|k alpha|alpha|m d]; try reflexivity; discriminate E1.
Qed.
